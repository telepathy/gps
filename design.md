...
- GPS 可自动从 PTIS 获取每个竖井的上一个发布版本，并自动递增 (如 `1.2.3` → `1.2.4`)
- 版本号也可在集中发版前由人工修改/确认
- 最终版本号以 GPS 中的确认版本为准
...
以仓库为粒度打 Tag（同一仓库的多个模块共享标签），仓库间并行操作：
1. clone 仓库到发布分支
2. 对该仓库下所有模块的目标版本打 tag
3. push tags

#### Phase 2: 依赖分析与拓扑排序

1. 调用 DAS API，传入所有的 module_id 列表和 DMS 依赖分支，获取模块间依赖关系
2. GPS 对依赖图进行拓扑排序，得到全序发布序列 `sorted_order`
3. 检测是否存在循环依赖；若存在则报错中止
...
- 对每个模块，检查其所有上游依赖是否已发布成功
  ...
  - 若池满或上游未就绪 → 阻塞等待
- 单个模块的发布流程（池中 worker 执行）：触发 CI/CD 流水线 → 轮询状态 → SUCCESS 则更新 DMS，FAILED 则按失败策略处理
- 所有模块处理完毕且池中无运行任务 → 结束
...
| 接口 | 方向 | 用途 |
|------|------|------|
| 获取依赖图 | GPS → DAS | 传入 module_id 列表和目标分支，返回模块间依赖关系（边集合），供 GPS 拓扑排序 |
...
DMS 管理模块制品的版本坐标（如 Maven 仓库 Nexus/Artifactory，或内部版本管理服务）。

| 接口 | 方向 | 用途 |
|------|------|------|
| 注册/更新制品版本 | GPS → DMS | 模块发布成功后，将新版本信息写入 DMS 对应分支 |
| 查询分支最新版本 | GPS → DMS | （可选）在发版前查询 DMS 某分支上某模块的当前最新版本 |
